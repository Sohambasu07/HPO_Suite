...
def rs_lcbench(
    ...
    logger.info("Running Random Search on LCBench Tabular Benchmark " "from MF Prior Bench\n")

    # Get the benchmark
    benchmark = get_benchmark(name="lcbench-tabular", task_id=task_id, datadir=datadir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Perform HPO using Random " "Search on LCBench Tabular Benchmark"
    )
    ...
    report = rs_lcbench(
        args.budget_type, args.budget, args.task_id, args.seed, args.datadir, args.save_dir
    )
